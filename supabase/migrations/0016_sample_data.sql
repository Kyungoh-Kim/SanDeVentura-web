...
--   bukhan  — 0 routes, 3 UNPROCESSED sessions → candidate_cells after match-and-aggregate
-- =============================================================================
...
values
  ...
  -- bukhan sessions (UNPROCESSED — no routes → cells become candidate_cells)
...
-- 6 points per session along 37.68°N 127.00°E → NE (inside bukhan bbox).
-- bukhan has NO routes → match-and-aggregate will save cells to candidate_cells.
-- H3 cells (res 11): bk0=8b30e1d0a999fff → bk5=8b30e1d0a802fff
-- Session 2 and 3 use < 0.5 m jitter to land in the same H3 cells as session 1.
...
values
  ...
  -- bukhan session 2 (slight jitter)
  ...
  -- bukhan session 3 (opposite jitter direction)
